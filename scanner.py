...
print("📡 [SISTEMA DETALHADO] Robô Over 0.5 HT monitorando com feedback de rede...", flush=True)
...
for loop in range(100):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("  🟢 API Titular (Foot-Data) respondeu com sucesso.", flush=True)
    except:
        pass

    # Se a titular falhar, exibe o aviso em amarelo e aciona o backup na hora
    if not jogos:
        try:
            print("  ⚠️ ALERTA: API Titular falhou ou oscilou. Acionando contingência...", flush=True)
            url_dinamica = f"{API_RESERVA}?t={int(time.time())}"
            response = requests.get(url_dinamica, timeout=4)
            if response.status_code == 200:
                jogos = response.json().get('data', [])
                print("  ✅ BACKUP OK: Dados extraídos com sucesso via API Reserva.", flush=True)
        except:
            print("  ❌ ERRO CRÍTICO: Ambas as APIs falharam neste ciclo.", flush=True)
            time.sleep(30)
            continue
