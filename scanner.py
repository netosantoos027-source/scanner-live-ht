...
# ---------------------------------------------------------------------
# PROJETO: ROBÔ OVER 0.5 HT (PRODUÇÃO VIA FEED GLOBAL LIVEDATA - CUSTO ZERO)
# ---------------------------------------------------------------------
TELEGRAM_TOKEN = "8977957095:AAFGcSuzjKxb2uX0lQzWwaozFdrreZ9myjc"
TELEGRAM_CHAT_ID = "@robo_over_05_ht"

# 🟢 NOVA ROTA OFICIAL: Feed dinâmico de alta disponibilidade em tempo real
API_URL = "https://esportes-livedata.com"
...
print("📡 [SISTEMA RESTAURADO] Robô Over 0.5 HT caçando jogos via LiveData...", flush=True)

# Loop contínuo de alta frequência (Roda por aproximadamente 50 minutos)
for loop in range(100):
    data_agora = datetime.now(fuso_br).strftime('%d-%m-%Y %H:%M:%S')
    print(f"🔄 Varrendo partidas... {data_agora}", flush=True)
    
    try:
        # Puxa os dados com cabeçalho limpo para a nova API aberta
        url_dinamica = f"{API_URL}?cache_bypass={int(time.time())}"
        response = requests.get(url_dinamica, timeout=5)
        
        if response.status_code != 200:
            print(f"  ⚠️ Servidor ocupado (Status {response.status_code}). Pulando ciclo...", flush=True)
            time.sleep(30)
            continue
        ...
        try:
            ...
            print("  ⚠️ Resposta inválida do servidor. Tentando novamente...", flush=True)
            time.sleep(30)
            continue
            
        jogos = dados_brutos.get('data', [])
        if not jogos:
            print("  ℹ️ Nenhuma partida ao vivo acontecendo neste minuto.", flush=True)
            time.sleep(30)
            continue
            
        print(f"  📊 Analisando {len(jogos)} jogos em andamento no mundo agora...", flush=True)
        ...
        print(f"❌ Falha na conexão com o servidor: {e}", flush=True)
        time.sleep(30)
